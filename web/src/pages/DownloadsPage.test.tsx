...
describe("DownloadsPage", () => {
  ...
  it("shows an error state with the fallback URL when fetch returns a non-ok response", async () => {
    ...
    const fallback = await waitFor(() =>
      screen.getByRole("link", { name: "https://unison-dumps.boidu.dev/dumps/latest.dump" }),
    )
    expect(fallback.getAttribute("href")).toBe("https://unison-dumps.boidu.dev/dumps/latest.dump")
